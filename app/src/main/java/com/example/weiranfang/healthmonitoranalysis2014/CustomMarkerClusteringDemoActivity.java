...
//        }
    ...
    protected void startDemo() {
        getMap().moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(40.7242, -74.1726), 11));
...
//        mClusterManager.addItem(new Person(position(), "Teach", R.drawable.teacher));
        ...
        new AddItemsTask().execute(new JSONSender());
    ...
    private class AddItemsTask extends AsyncTask<JSONSender,Long,JSONArray> {
        @Override
        protected JSONArray doInBackground(JSONSender... params) {
            return params[0].GetArrayByID(24);
        }
        ...
        protected void onPostExecute(JSONArray jsonArray) {
            ...
            for(int i = 0; i < jsonArray.length(); i++) {
                try {
                    ...
                    String picUrl = object.getString("profile_image_url");
                    double lat = object.getDouble("geo_lat");
                    double lng = object.getDouble("geo_long");

                    Bitmap picture = new ImageLoadTask(picUrl).execute().get();

                    mClusterManager.addItem(new Person(new LatLng(lat, lng), name, picture));
